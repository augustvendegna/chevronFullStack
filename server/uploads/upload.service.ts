...
export class UploadService {
  ...
  */
  ...
  /*
  upload(selectedFile: File): Observable<HttpEvent<any>> {
    const formData: FormData = new FormData();

    formData.append('file', selectedFile);

    const req = new HttpRequest('POST', `${this.configUrl}/addSubmission`, formData, {
      reportProgress: true,
      responseType: 'json'
    });
  ...
  getFiles(): Observable<any> {
    return this.httpClient.get(`${this.configUrl}/uploads`);
  }
  */
